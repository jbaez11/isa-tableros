  ...
  <div class="app">
    <v-app>
      <v-main>
        ...
        <v-container class="">
          <v-row>
            <v-col style="justify-content: center; align-items: center;text-align: center;">
              
                <h2 class="orange--text">Fecha <span style="color:gray;">a</span>  auditar</h2>
                  <v-date-picker color="orange accent-3 lighten-1" v-model="date"></v-date-picker>
                  <!-- <p>{{date}}</p>-->
                  <p v-show="false">{{ submitDate }}</p>
              
            </v-col>
            <v-col style="justify-content: center; align-items: center;text-align: center;">
                  <h3 class="orange--text">Cantidad <span style="color:gray;">Grabaciones</span></h3>
                  <span class="text-h2 orange--text">{{ this.cantidadLlamadas }}</span>
        ...
        <v-simple-table class="mt-5" v-show="topByCategory != false" >
          <template>
            ...
            <tbody>
               <tr  
                v-for="agentAudit in topByCategory"
                :key="agentAudit.name"
              >
                <td>{{agentAudit.name}}</td>
                <td style="text-align: center;">{{agentAudit.results.records}}</td>
                <td style="text-align: center;">{{agentAudit.results["infaltables positivas"]}}</td>
                <td style="text-align: center;">{{agentAudit.results["infaltables negativas"]}}</td>
                <td style="text-align: center;">{{agentAudit.results["no permitidas positivas"]}}</td>
                <td style="text-align: center;">{{agentAudit.results["no permitidas negativas"]}}</td>
                <td style="text-align: center;">{{agentAudit.results["recomendaciones positivas"]}}</td>
                <td style="text-align: center;">{{agentAudit.results["recomendaciones negativas"]}}</td>
        ...
        <v-simple-table class="mt-5" v-show="topByCategory != false">
          <template>
            ...
            <tbody>
              ...
              >
                <td>{{agentAudit.name}}</td>
                <td style="text-align: center;">{{agentAudit.results.infaltable.positive}}</td>
                <td style="text-align: center;">{{agentAudit.results.infaltable.negative}}</td>
                <td style="text-align: center;">{{agentAudit.results["no permitida"].positive}}</td>
                <!-- <td>{{agentAudit.results["no permitida"].negative}}</td> -->
                <td style="text-align: center;">{{agentAudit.results["recomendación"].positive}}</td>
    ...
    async mostrar() {
      ...
      for (let i = 0; i < this.auditAgents.length; i++) {
        ...
        for (let keyword in this.auditAgents[i].keywords) {
          ...
          if (haveResults) {
            if (!("positive" in recordsByCategory[nameAgent][keyfile][category])) {
              recordsByCategory[nameAgent][keyfile][category]["positive"] = 1;
            } else {
              recordsByCategory[nameAgent][keyfile][category]["positive"] += 1;
            }
          } else {
            if (!("negative" in recordsByCategory[nameAgent][keyfile][category])) {
        ...
        for(let keyfile in recordsByCategory[agent]){
          ...
          console.log("key",keyfile)
           if("positive" in (recordsByCategory[agent][keyfile])["infaltable"]){
             (topByCategory[agent])["infaltables positivas"]+= recordsByCategory[agent][keyfile]["infaltable"]["positive"];
           }
           if("negative" in recordsByCategory[agent][keyfile]["infaltable"]){
             (topByCategory[agent])["infaltables negativas"]+= recordsByCategory[agent][keyfile]["infaltable"]["negative"] ;
           }
           if("positive" in recordsByCategory[agent][keyfile]["no permitida"]){
             (topByCategory[agent])["no permitidas positivas"]+=recordsByCategory[agent][keyfile]["no permitida"]["positive"] ;
           }
           if("negative" in recordsByCategory[agent][keyfile]["no permitida"]){
             (topByCategory[agent])["no permitidas negativas"]+=recordsByCategory[agent][keyfile]["no permitida"]["negative"] ;
           }
           if("positive" in recordsByCategory[agent][keyfile]["recomendación"]){
             (topByCategory[agent])["recomendaciones positivas"]+=recordsByCategory[agent][keyfile]["recomendación"]["positive"] ;
           }
           if("negative" in recordsByCategory[agent][keyfile]["recomendación"]){
             (topByCategory[agent])["recomendaciones negativas"]+=recordsByCategory[agent][keyfile]["recomendación"]["negative"];
           }
